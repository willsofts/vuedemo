...
<template>
  ...
  <div class="pt-page pt-page-current pt-page-controller search-pager">
    <PageHeader
      ...
      :build="buildVersion"
      :visible="displayPageHeader"
    />
    <SearchForm
      ref="searchForm"
      :labels="labels"
      :dataCategory="dataCategory"
      @data-select="dataSelected"
    ...
    loadDataCategories(loading, callback) {
      ...
      $.ajax({
        ...
        error: function (transport, status, errorThrown) {
          console.error(
            "loadDataCategories: error: status",
            status,
            "errorThrown",
            errorThrown
          );
    ...
    resetDataCategories(lang) {
      ...
      if (kt_marrystatus) {
        marrystatus = kt_marrystatus.map((item) => {
          return { id: item.statusid, text: "TH" == lang ? item.nameth : item.nameen };
        });
      }
      let kt_languages = this.dataChunk["kt_languages"];
      if (kt_languages) {
        languages = kt_languages.map((item) => {
          return { id: item.langid, text: "TH" == lang ? item.nameth : item.nameen };
        });
      }
